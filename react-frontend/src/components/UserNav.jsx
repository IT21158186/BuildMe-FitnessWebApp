                ...
                <Toolbar disableGutters>
                    ...
                    <Box sx={{ flexGrow: 1, display: { xs: isNavOpen ? 'block' : 'none', md: 'flex' } }}>
                        {pages.map((page) => (
                            <Button
                                ...
                                to={page === 'Home' ? '/' : `/${page.toLowerCase()}`} // Set the path accordingly
                    ...
                    <Box sx={{ flexGrow: 0 }}>
                        <Tooltip title="Open settings">
                            <IconButton sx={{ p: 0 }}>
                                <Avatar alt="Remy Sharp" src="/static/images/avatar/2.jpg" onClick={handleOpenUserMenu} />
                            </IconButton>
                        </Tooltip>
                        <Menu
                            sx={{ mt: '45px' }}
                            ...
                            {settings.map((setting) => (
                                <MenuItem key={setting} onClick={handleCloseUserMenu}>
                                    <Typography textAlign="center">
                                        <Link to={setting === 'Login' ? '/login' : `/${setting.toLowerCase()}`}>{setting}</Link>
                                    </Typography>
                                </MenuItem>
                            ))}
                        </Menu>
